...
if __name__=="__main__":
    ...
    device='cuda'
    ppo=PPO(learning_rate=0.001, clipping_ratio=0.2, job_len=130,machine_len=12, d_model=512, num_heads=8, fea_len=6,num_layers=2,dim_feedforward=1024).to(device)
    PMSP=PMSPScheduler(num_machines=12, initial_jobs=80, additional_jobs=10, additional_arrivals=5, processing_time_range=(5, 15), machine_speed=[1,1,1,1,1,1,1.5,1.5,1.5,1.5,1.5,1.5], start_additional_arrival=20, arrival_interval=10,setup_range=(5,15),family_setup_num=10)
    ...
    number_of_iteration=int(10001/number_of_trial)  # 전체 iteration #iteration 단위로 문제 변화
    validation=[]
    validation_step = 100
    num_of_meta=6
    history = np.zeros((number_of_iteration * number_of_trial,2))
    validation_history=np.zeros((int(10001/validation_step)+10,number_of_validation))
    ...
    for i in range(number_of_iteration):
        ...
        total_tardy,ave_tardy,episode=PMSP.run_simulation(number_of_problem,number_of_batch,ppo)
        ...
        if i%validation_step==0:
            ...
            for j in range(number_of_validation):
                ...
                for k in range(number_of_validation_batch):
                    ...
                    episode=[]
                    machines,jobss,start_times,durations,episode,total_tardy,total_reward=PMSP.schedule_jobs(jobs,episode,ppo)
