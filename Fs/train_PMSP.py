...
if __name__=="__main__":
    ...
    ppo=PPO(learning_rate=0.001, clipping_ratio=0.2, machine_len=10, d_model=512, num_heads=8, fea_len=25,num_layers=1,dim_feedforward=1024).to(device)

    number_of_validation=10
    number_of_validation_batch=100
    number_of_problem=3# 한번에 몇개의 문제를
    number_of_batch=6 # 문제당 몇 episode씩 한번에 학습할껀지
    number_of_trial=1  #1, 10, 100, 1000 #이를 몇번 반복할껀지
    number_of_iteration=int(2/number_of_trial)  # 전체 iteration #iteration 단위로 문제 변화
    validation=[]
    validation_step = 100
    num_of_meta=6
    history = np.zeros((number_of_iteration * number_of_trial,2))
    validation_history=np.zeros((int(2/validation_step)+10,number_of_validation))
