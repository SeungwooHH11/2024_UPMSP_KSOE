...
if __name__=="__main__":
    ...
    ppo=PPO(learning_rate=0.001, clipping_ratio=0.2, machine_len=6, d_model=512, num_heads=4, fea_len=13,num_layers=1,dim_feedforward=512).to(device)

    number_of_validation=10
    number_of_validation_batch=10
    number_of_problem=4# 한번에 몇개의 문제를
    number_of_batch=8 # 문제당 몇 episode씩 한번에 학습할껀지
    number_of_trial=1  #1, 10, 100, 1000 #이를 몇번 반복할껀지
    number_of_iteration=int(201/number_of_trial)  # 전체 iteration #iteration 단위로 문제 변화
    validation=[]
    validation_step = 40
    num_of_meta=6
    history = np.zeros((number_of_iteration * number_of_trial,2))
    validation_history=np.zeros((int(201/validation_step)+10,number_of_validation))
    ...
    mode_list = ['SSPT', 'ATCS', 'MDD', 'COVERT','SSST','SSSPT']
    ...
    for e,priority in enumerate(mode_list):
        ...
        for j in range(number_of_validation):
            ...
            for k in range(number_of_validation_batch):
                ...
                episode=[]
                episode,total_tardy,total_reward=PMSP.schedule_jobs(jobs,setup,episode,ppo,priority)
